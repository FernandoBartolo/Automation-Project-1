...
describe.only('This is first test suite, Fernando Bartolo', () => {
    it.only('User can submit data only when valid mandatory values are added', () => {
        cy.get('#username').type('FernBart')
        cy.get('#firstName').type('Fernando')
        cy.get('#lastName').type('Bartolo')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('testpass123')
        cy.get('[name="confirm"]').type('testpass123')
    ...
    */
    ...
    it.only('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('#firstName').type('Fernando')
        cy.get('#lastName').type('Bartolo')
        cy.get('[data-testid="phoneNumberTestId"]').type('12345677')
        cy.get('input[name="password"]').type('testpass123')
        cy.get('[name="confirm"]').type('testpass123')   
    ...
    it.only('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('#lastName').type('Bartolo')
        cy.get('[data-testid="phoneNumberTestId"]').type('12345677')
        cy.get('input[name="password"]').clear('testpass123')
        cy.get('[name="confirm"]').clear('testpass123')   
    ...
    it.only('User cannot add letters to phone number', () => {
        ...
        cy.get('#firstName').type('Fernando')
        cy.get('#lastName').type('Bartolo')
        cy.get('[data-testid="phoneNumberTestId"]').type('fernando')
        cy.get('input[name="password"]').type('testpass123')
        cy.get('[name="confirm"]').type('testpass123')   
